...
최근 MZ세대를 중심으로 ‘GPS 아트’가 러닝 문화로 자리잡고 있습니다.  
SNS를 보면 ‘댕댕런’, ‘고구마런’, ‘고래런’처럼 **지도 위에 귀여운 그림을 완성하며 뛰는 러닝 챌린지**가 유행하고 있습니다.

사용자들은 나이키 런클럽(NRC), 스트라바(Strava) 등의 앱을 활용해  
**지도 위에 그림이 되도록 직접 경로를 설계하고, 그 경로대로 뛰며 완성된 러닝 결과를 공유**합니다.

하지만 기존 방식은 다음과 같은 불편함이 있습니다:
- 어떤 그림이 완성될지 **뛰어보기 전에는 알 수 없으며**  
- 원하는 모양을 만들기 위해서는 **지도 위에서 직접 동선을 설계하거나**,  
  **매번 방향을 바꿔가며 감으로 뛰어야 하는 번거로움**이 존재합니다.

<p align="center">
  <img src="https://github.com/user-attachments/assets/cecf4737-dfd4-44d1-926e-672ea33d06ed" width="550" alt="직접 GPS 아트 설계 예시"/>
</p>

**DrawingRun**은 이러한 불편을 해결하기 위해 만들어진 앱으로,  
- 사용자가 지도 위에 **직접 그림을 그려 러닝 경로로 저장**하거나,  
- **미리 저장된 경로(예: 별, 고양이 등 라벨화된 경로)를 선택해** 러닝할 수 있습니다.

즉, **뛰기 전부터 어떤 그림이 완성될지 알 수 있고**,  
지도 분석이나 복잡한 설계 없이도 **자유롭고 창의적인 GPS 아트를 완성**할 수 있습니다.

**DrawingRun**은 창의적인 표현과 운동을 결합하여  
**러닝의 동기를 부여하고, 즐거움을 확장하는 사용자 경험**을 제공합니다.
...
| 1991200  | 전경호 | UI 디자인 / 시각 스타일 설계 / 보안 기능         |
| 2171042  | 손지형 | DB 관리 / 드로잉 기능 구현             |
| 2171087  | 박수빈 | 경로 리스트 구성 및 그림 선택 유도 UI 설계 / 그림 인식 모델 시도 및 전처리 / 보안 기능 |
